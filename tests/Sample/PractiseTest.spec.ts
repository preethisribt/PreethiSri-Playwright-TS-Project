...
test("Keyboard and Mouse", async ({ page }) => {
  await page.goto("https://omayo.blogspot.com/");
  await page.getByText("This blog is created for").getByRole("link", { name: "http://www.Selenium143." }).click({
    button: "right"
  });
...
test("Frames1", async ({ page }) => {
  ...
  await childFrame.getByPlaceholder("Enter email").fill("childsample@gmail.com");
  await page.pause();
});
...
test("Frames2", async ({ page }) => {
  ...
  await expect(frame.getByText("What should I know prior to learning Selenium?")).toBeVisible();
  await page.frameLocator("iframe[id='navbar-iframe']").locator("input#b-query").fill("Sample");
});
